...
export class FilterPipe implements PipeTransform {
  ...
  transform(items: any[], searchText: string, filterGroup: any): any[] {
    ...
    return _.filter(items, (it) =>
      this.filterByOr(it, filterGroup, searchText)
    );
  }
  ...
  filterByOr(item: any, filterGroup: any, searchText: string) {
    ...
    for (let key of filterGroup) {
      ...
      switch (typeof item[key]) {
        case 'string':
          flag = this.filterByString(item, key, searchText);
          break;
        case 'number':
          flag = this.filterByNumber(item, key, searchText);
          break;
      }
      if (flag) {
        break;
