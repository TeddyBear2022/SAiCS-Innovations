...
import { MerchCategory } from 'src/app/Models/MerchCategory';
import { ApiService } from 'src/app/Services/api.service';

@Component({
  selector: 'app-merch-cat',
  templateUrl: './merch-cat.component.html',
  styleUrls: ['./merch-cat.component.scss'],
})
export class MerchCatComponent implements OnInit {
  ...
  LoadInfo() {
    this.api.GetMerchCat().subscribe((data) => {
      ...
      {
         this.lookUpArr.push({
          id: d.merchCategoryId,
          name: d.merchCategoryName
         })
      }
  ...
  updateEditCache(): void {
    ...
    SendData()
    {
      ...
      else
      {
        ...
        console.log(value)
         this.api.CreateLookUp("MerchCategory", value).subscribe((res) => {
  ...
  saveEdit(id: string): void {
    ...
    {
    let update = {} as MerchCategory;
    update.merchCategoryId = item.id
    update.merchCategoryName = this.editCache[id].data.name

    this.api.UpdateMerchCategory(update).subscribe(res =>{
      if(res.body == "Success")
      {
        this.isEditClicked = false;
        this.lookUpArr.length = 0
  ...
  cancelEdit(id: string): void {
    ...
    remove(index: number) {
      this.api.DeleteLookUp("MerchCategory",index).subscribe((res) => {
        if(res.body == 1 || res.body == 0)
        {
          this.lookUpArr.length = 0
          this.LoadInfo()
