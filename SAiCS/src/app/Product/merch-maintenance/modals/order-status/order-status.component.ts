...
export class OrderStatusComponent implements OnInit {
  ...
  //Loadsection
  LoadInfo() {
    this.api.MerchOrderStatuses().subscribe((res) => {
  ...
  SendData()
  {
    ...
    else
    {
      ...
      console.log(value)
       this.api.CreateLookUp("OrderStatus", value).subscribe((res) => {
...
saveEdit(id: string): void {
  ...
  {
  let update = {} as OrderStatus;
  update.OrderStatusId = item.id
  update.OrderStatusName = this.editCache[id].data.name

  this.api.EditOrderStatus(update).subscribe(res =>{
    if(res.body == "Success")
    {
      this.isEditClicked = false;
      this.lookUpArr.length = 0
...
cancelEdit(id: string): void {
  ...
  remove(index: number) {
    this.api.DeleteLookUp("OrderStatus",index).subscribe((res) => {
      if(res.body == 1 || res.body == 0)
      {
        this.lookUpArr.length = 0
        this.LoadInfo()
