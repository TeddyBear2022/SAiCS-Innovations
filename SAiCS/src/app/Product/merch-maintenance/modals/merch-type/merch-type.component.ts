...
import { MerchType } from 'src/app/Models/MerchType';
import { ApiService } from 'src/app/Services/api.service';

@Component({
  selector: 'app-merch-type',
  templateUrl: './merch-type.component.html',
  styleUrls: ['./merch-type.component.scss'],
})
export class MerchTypeComponent implements OnInit {
  ...
  SendData()
  {
    ...
    else
    {
      ...
      console.log(value)
       this.api.CreateLookUp("MerchType", value).subscribe((res) => {
...
saveEdit(id: string): void {
  ...
  {
  let update = {} as MerchType;
  update.merchTypeId = item.id
  update.merchTypeName = this.editCache[id].data.name

  this.api.UpdateMerchType(update).subscribe(res =>{
    if(res.body == "Success")
    {
      this.isEditClicked = false;
      this.lookUpArr.length = 0
...
cancelEdit(id: string): void {
  ...
  remove(index: number) {
    this.api.DeleteLookUp("MerchType",index).subscribe((res) => {
      if(res.body == 1 || res.body == 0)
      {
        this.lookUpArr.length = 0
        this.LoadInfo()
