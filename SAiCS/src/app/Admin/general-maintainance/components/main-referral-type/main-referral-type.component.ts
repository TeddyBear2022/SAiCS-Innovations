...
import { ReferralLinkType } from 'src/app/Models/ReferralLinkType';
import { ApiService } from 'src/app/Services/api.service';

@Component({
  selector: 'app-main-referral-type',
  templateUrl: './main-referral-type.component.html',
  styleUrls: ['./main-referral-type.component.scss'],
})
export class MainReferralTypeComponent implements OnInit {
...
//Loadsection
LoadInfo() {
  this.api.GetReferralTypes().subscribe((res) => {
    let data: any = res
    console.log(data);
...
SendData()
{
  ...
  else
  {
    ...
    console.log(value)
     this.api.CreateLookUp("ReferralLinkType", value).subscribe((res) => {
...
{
  let update = {} as ReferralLinkType;
  update.ReferralLinkTypeID = item.id
  update.ReferralType = this.editCache[id].data.name
 this.api.UpdateReferralType(update).subscribe(res =>{
  if(res.body == "Success")
  {
    this.isEditClicked = false;
    this.lookUpArr.length = 0
...
remove(index: number) {
  this.api.DeleteLookUp("ReferralLinkType",index).subscribe((res) => {
    if(res.body == 1 || res.body == 0)
    {
      this.lookUpArr.length = 0
      this.LoadInfo()
