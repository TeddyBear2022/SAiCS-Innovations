...
@Component({
  selector: 'app-main-bank',
  templateUrl: './main-bank.component.html',
  styleUrls: ['./main-bank.component.scss'],
})
export class MainBankComponent implements OnInit {
...
//Loadsection
LoadInfo() {
  this.api.GetAllBanks().subscribe((res) => {
    let data: any = res
    console.log(data);
...
SendData()
{
  ...
  else
  {
    ...
    console.log(value)
     this.api.CreateLookUp("Bank", value).subscribe((res) => {
...
{
  let update = {} as Bank;
  update.BankId = item.id
  update.BankName = this.editCache[id].data.name
 this.api.UpdateBank(update).subscribe(res =>{
  if(res.body == "Success")
  {
    this.isEditClicked = false;
    this.lookUpArr.length = 0
...
remove(index: number) {
  this.api.DeleteLookUp("Bank",index).subscribe((res) => {
    if(res.body == 1 || res.body == 0)
    {
      this.lookUpArr.length = 0
      this.LoadInfo()
