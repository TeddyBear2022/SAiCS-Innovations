...
import { FAQType } from 'src/app/Models/FAQType';
import { ApiService } from 'src/app/Services/api.service';

@Component({
  selector: 'app-main-faq-type',
  templateUrl: './main-faq-type.component.html',
  styleUrls: ['./main-faq-type.component.scss'],
})
export class MainFaqTypeComponent implements OnInit {
...
//Loadsection
LoadInfo() {
  this.api.GetFAQTypes().subscribe((res) => {
    let data: any = res
    console.log(data);
...
SendData()
{
  ...
  else
  {
    ...
    console.log(value)
     this.api.CreateLookUp("FAQType", value).subscribe((res) => {
...
{
  let update = {} as FAQType;
  update.FaqtypeId = item.id
  update.FaqtypeName = this.editCache[id].data.name
 this.api.UpdateFaqType(update).subscribe(res =>{
  if(res.body == "Success")
  {
    this.isEditClicked = false;
    this.lookUpArr.length = 0
...
remove(index: number) {
  this.api.DeleteLookUp("FAQType",index).subscribe((res) => {
    if(res.body == 1 || res.body == 0)
    {
      this.lookUpArr.length = 0
      this.LoadInfo()
