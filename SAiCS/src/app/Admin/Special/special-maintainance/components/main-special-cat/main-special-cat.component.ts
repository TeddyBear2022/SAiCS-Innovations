...
@Component({
  selector: 'app-main-special-cat',
  templateUrl: './main-special-cat.component.html',
  styleUrls: ['./main-special-cat.component.scss'],
})
export class MainSpecialCatComponent implements OnInit {
  ...
  //Loadsection
  LoadInfo() {
    this.api.GetSpecialCategory().subscribe((res) => {
      //console.log(res);
      let data: any = res
      for(let d of data)
      {
         this.lookUpArr.push({
          id: d.specialCategoryId,
          name: d.specialCategoryName
         })
      }
  ...
  SendData()
  {
    ...
    else
    {
      ...
      console.log(value)
       this.api.CreateLookUp("SpecialCategory", value).subscribe((res) => {
  ...
  {
   this.api.UpdateSpecialCategory(item.id, this.editCache[id].data.name).subscribe(res =>{
    if(res.body == "Updated")
    {
      this.isEditClicked = false;
      this.lookUpArr.length = 0
  ...
  remove(index: number) {
    this.api.DeleteLookUp("SpecialCategory",index).subscribe((res) => {
      if(res.body == 1 || res.body == 0)
      {
        this.lookUpArr.length = 0
        this.LoadInfo()
