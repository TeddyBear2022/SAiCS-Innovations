...
@Component({
  selector: 'app-main-special-type',
  templateUrl: './main-special-type.component.html',
  styleUrls: ['./main-special-type.component.scss'],
})
export class MainSpecialTypeComponent implements OnInit {
...
SendData()
{
  ...
  else
  {
    ...
    console.log(value)
     this.api.CreateLookUp("SpecialType", value).subscribe((res) => {
...
{
 this.api.UpdateSpecialType(item.id, this.editCache[id].data.name).subscribe(res =>{
  if(res.body == "Updated")
  {
    this.isEditClicked = false;
    this.lookUpArr.length = 0
...
remove(index: number) {
  this.api.DeleteLookUp("SpecialType",index).subscribe((res) => {
    if(res.body == 1 || res.body == 0)
    {
      this.lookUpArr.length = 0
      this.LoadInfo()
